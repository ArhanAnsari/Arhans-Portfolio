...
const DEFAULT_INTERNAL_PAGES = {
  about: {
    title: 'About ArhanOS',
    content: 'ArhanOS Portfolio is a macOS-inspired interactive portfolio environment.',
  },
  projects: {
    title: 'Projects',
  ...
  skills: {
    title: 'Skills',
    content: 'Frontend, backend, AI, 3D, and product design experiences live here.',
  },
  terminal: {
    title: 'Terminal',
...
const normalizeUrlInput = (value) => {
  ...
  if (/^https?:\/\//i.test(input) || /^mailto:/i.test(input) || /^about:/i.test(input)) {
    return input;
  }
    ...
    (set, get) => ({
      ...
      closeTab: (tabId) =>
        set((state) => {
          ...
          const activeTabId = state.activeTabId === tabId ? tabs[tabs.length - 1]?.id || tabs[0].id : state.activeTabId;
      ...
      setTabError: (tabId, error) =>
        set((state) => ({
          tabs: state.tabs.map((tab) => (tab.id === tabId ? { ...tab, loading: false, error } : tab)),
        })),

      addBookmark: (bookmark) =>
        set((state) => ({
          bookmarks: [
            ...state.bookmarks.filter((entry) => entry.title !== bookmark.title && entry.url !== bookmark.url),
